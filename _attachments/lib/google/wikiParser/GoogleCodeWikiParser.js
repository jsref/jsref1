...
function GoogleCodeWikiParser() {
    ...
    this.rx = {
        ...
        headers: [
            ...
            [/^\s*#sidebar.*$/i, function() {
                return ps.getWarning("The `#sidebar` directive is not supported by this parser!")
            }]
        ],
        ...
        block: {
            ...
            table: {
                ...
                doLine: function(line, result) {
                    ...
                    if (cells) {
                        cells.shift();
                    } // strip empty element after final separator.
                    ...
                    for (var i in cells) {
                        if (!cells.hasOwnProperty(i)) {
                            continue;
                        } // Prototype-specific kludge. The fuckers.
                        c = cells[i];
                        ...
                        out
                        .push('<', tag, ' style="border: 1px solid #aaa; padding: 5px;">', // hard-coded values taken from Google Code
                        ps.parseInlined(c), '</', tag, '>');
                    }
                    out.push('</tr>');
            ...
            code: {
                ...
                doLine: function(line, result) {
                    ...
                    if ((undefined === line) || (m = /(.*)}}}(.*)/.exec(line))) {
                        ...
                        result.push('</pre><br>');
                        if (m && m[2]) {
                            result.push(ps.parseInlined(m[2]));
                        }
                    ...
                    if (1 == ps.inCode) {
                        ...
                        result.push((line && line.length) ? line.replace(/&/g, '&amp;').replace(/</g, '&lt;') : '');
            ...
            listOLUL: {
                ...
                doLine: function(line, result, tag) {
                    ...
                    if ((sp.length > prevSpace.length)) {
                        ...
                        result.push('<' + tag + '>');
                        ps.liSpaces.push(sp);
                    } else if (prevTag != tag) { // special case: different list types at same indention level.
                        result.push('</' + ps.liTags[ps.liTags.length - 1] + '>');
                        ps.liTags[ps.liTags.length - 1] = tag;
                        result.push('<' + tag + '>');
                    }
                    result.push('<li>'+ ps.parseInlined(m[3]) + '</li>');
                    //result.push('<li' +' debug="ps.listLevel='+ps.listLevel+' spaces.length='+sp.length+' tag='+tag+'"' + '>' + ps.parseInlined(m[3]) + '</li>');
                    return 1;
                }
            }/*listOLUL*/,
            ...
            blockquote: {
                ...
                doLine: function(line, result) {
                    ...
                    if (ps.inBQ == 1) {
                        ...
                        //m = this.rx.exec(line);
                        if (m && m[1]) {
                            result.push(ps.parseInlined(m[1]));
                        }
                        //print("STARTING CODE:",line);
                    } else {
...
GoogleCodeWikiParser.prototype.headerReplace = function headerReplace(lvl, $1) {
    var norm = $1.replace(/\s+/g, '_').replace(/["`]/g, '') // FIXME: confirm exactly which chars GoCoWi removes!;
    this.hList.push({
        level: lvl,
        name: $1,
        href: '#' + norm
...
GoogleCodeWikiParser.prototype.getWarning = function(text) {
    return this.options
    .disableWarnings ? '' : '<span style="color:red;background-color:yellow;">WIKI PARSE WARNING: ' + text + '</span>';
};
...
GoogleCodeWikiParser.prototype.parseLineVerbatim = function(text) {
    ...
    for (i = 0; i < end; ++i) {
        ...
        buf = buf.replace(/&/, '&amp;')
        .replace(/</g, '&lt;')// Keep further inline markup within this block from being parsed later on:
    ...
    for (ch = 0,i = 0; i < end; ++i) {
        ...
        // [WikiWord(\s+description)?], [http://...(\s+description)?]
            ...
            if (']' !== ch) {
                ...
                tag = (sp && (sp.length > 1)) ? buf.substr(sp[0]
                .length) : (sp ? sp[0] : 'WIKI_PARSING_ERROR'/*should never happen!*/);
...
GoogleCodeWikiParser.prototype.renderTOC = function(maxDepth) {
    ...
    function render(h) {
        var sp = '';
        for (var x = 0; x < h.level; ++x) {
            sp += '  ';
        }
        var a = "* <a href='" + h.href + "'"
            //+" SPACE="+sp.length + ">" + ps.parseInlined(h.name) + "</a>";
        out.push(sp + a);
    }
...
GoogleCodeWikiParser.prototype.parse = function(text) {
    ...
    for (i = 0; i < end; ++i) {
        ...
        for (k in this.rx.block) {
            if (! this.rx.block.hasOwnProperty(k)) {
                continue;
            } // i fucking hate the Prototype lib developers for this.
            ...
            if (rc < 0) {
                --i;
            } // back up and try the line again.
            if (undefined !== rc) {
                break;
            }
        }
                ...
                if (bl.rx.test(ln)) {
                    ...
                    for (k in res) {
                        if (! res.hasOwnProperty(k)) {
                            continue;
                        } // Protoype-specific kludge. Assholes!!!!
                        out.push(res[k]);
                    }
                    res = null;
                    if (rc < 0) {
...
exports.Parser = GoogleCodeWikiParser;
